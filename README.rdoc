...
you reusable API wrapper methods with the exact behavior you need. It goes something like this:
  ...
  module YourLibModule
    ...
    function :FindWindow [:pointer, :pointer], :ulong, zeronil: true   # forces function to return nil if nothing found

    function :GetWindowText, [ :pointer, :pointer, :int ], :int do |api, handle| # your own method extension in a block
      buffer = FFI::MemoryPointer.new :char, 1024
      buffer.put_string(0, "\x00" * 1023)
      num_chars = api.call(handle, buffer, 1024)
      return nil if num_chars == 0
      string = buffer.get_bytes(0, num_chars)
    end
  end

  handle = find_window(nil, nil) # find ANY window
  puts window_text(handle)       # print its title
